...
#define CHUCK_EOF        0x454F4643UL /* "EOFC" */

// On Disk struct (packed, explicit padding)
/*
 * File header > first 32 bytes of every .crap file
...
typedef struct __attribute__((packed)) {
  ...
  uint8_t  stream_count;    // total num of stream
  uint8_t  _pad[3];
  int64_t  duration_us;     // total duration in micro sec's
  uint64_t metadata_offset; // byte offset to META chuck 
  uint64_t index_offset;    // byte offset to FRIX chuck
  uint32_t header_crc32;    // CRC32 of bytes [0..27]
} CrapFileHeader;           // 40 bytes total
    
/*
 * Generic chunk header, precedes every chunk's payload
*/
...
typedef struct __attribute__((packed)) {
  ...
  uint16_t codec_id;       // codec indentifer
 ...
 * FRIX payload = arr of this + uint32_t entry_count prefix
*/

typedef struct __attribute__((packed)) {
  int64_t  pts;
  uint64_t byte_offset; // abs offset of FRMD chunk
  uint8_t  stream_id;
  uint8_t  frame_type;
  uint8_t  _pad[2];
} CrapIndexEntry // 22 bytes
...
typedef struct {
  ...
  CrapIndexEntry *index; // heap allocated arr 
...
int crap_read_frame (CrapContext *ctx, CrapFrameHeader *fh, uint8_t *buf, uint32_t buf_size);
int crap_write_frame(CrapContext *ctx, const CrapFrameHeader *fh, const uint8_t *data);
