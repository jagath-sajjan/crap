...
/*
 * Write a chunk: [CrapChunkHeader][payload][crc32 of payload]
 */
...
/*
 * read and validate a chunk header; leave fp at start of payload
 * caller must seek past payload + 4 byte CRC when done
 */
...
int crap_open_read(CrapContext *ctx, const char *path) {
    ...
    if (ctx->header.magic != CRAP_MAGIC) { fclose(ctx->fp); return CRAP_ERR_MAGIC; }
    ...
    if (computed != ctx->header.header_crc32) { fclose(ctx->fp); return CRAP_ERR_CRC; }

    // parse stream info chunks
    ...
    for (int i = 0; i < ctx->header.stream_count; i++) {
        ...
        computed_crc = crc32((const uint8_t *)&si, sizeof(si));
        if (read_exact(ctx->fp, &stored_crc, 4) != CRAP_OK) goto fail_io;
        if (stored_crc != computed_crc) { fclose(ctx->fp); return CRAP_ERR_CRC; }
    ...
    // load frame index
    if (ctx->header.index_offset == 0) return CRAP_OK; /* no index yet */
    ...
    // validate CRC over count + entries
    ...
    uint32_t stored_crc, computed_crc;
    computed_crc = crc32(crc_buf, 4 + index_bytes);
    free(crc_buf);
    if (read_exact(ctx->fp, &stored_crc, 4) != CRAP_OK) { free(ctx->index); goto fail_io; }
    if (stored_crc != computed_crc) { free(ctx->index); fclose(ctx->fp); return CRAP_ERR_CRC; }
...
int crap_open_write(CrapContext *ctx, const char *path) {
    ...
    /* write placeholder header caller fills fields then calls
       crap_finalize() which rewrites it with correct offsets/CRC */
    ...
    // leave offsets 0; will be patched on finalize
    ctx->header = hdr;
...
int crap_write_frame(CrapContext *ctx, const CrapFrameHeader *fh,
                     const uint8_t *data) {
    // payload = CrapFrameHeader + encoded bytes
    ...
    int ret = write_chunk(ctx->fp, CHUNK_FRAMEDATA, payload, (uint64_t)payload_size);
    free(payload);
    return ret;
}
...
int crap_read_frame(CrapContext *ctx, CrapFrameHeader *fh,
    ...
    // read and validate CRC over full payload
    size_t payload_size = sizeof(CrapFrameHeader) + fh->data_size;
    ...
    uint32_t computed_crc = crc32(tmp, payload_size);
    free(tmp);

    uint32_t stored_crc;
    if (read_exact(ctx->fp, &stored_crc, 4) != CRAP_OK) return CRAP_ERR_IO;
    if (stored_crc != computed_crc) return CRAP_ERR_CRC;
...
int crap_seek_pts(CrapContext *ctx, uint8_t stream_id, int64_t pts) {
    ...
    // binary search for nearest I-frame at or before pts
